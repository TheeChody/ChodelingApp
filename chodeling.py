...
async def top_bar(left_side: str) -> str:
    try:
        ...
        dashes = f"{colour('purple' if xp_perc > 0 else 'normal', str(level_check - 1))}{colour('purple', '-' * (xp_perc - len(str(level_check - 1)) - xp_boost))}{colour('blue', '-' * xp_boost)}{'-' * (len(long_dashes) - len(str(level_check)) - xp_perc)}{colour('blue' if xp_boost > 0 else 'normal', str(level_check))}"
